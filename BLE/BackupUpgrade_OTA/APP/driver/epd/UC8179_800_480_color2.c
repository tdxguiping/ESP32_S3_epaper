...
void Display_EPD_Driver(void)
{
    EPD_W21_WriteCMD(0x17);         // 自动更新指令   
    EPD_W21_WriteDATA(0xA5);        // PON DRF POF DSLP 屏自动更新后进入深度睡眠    
...
void Init_EPD_Driver(void)
{
    ...
    //EPD_W21_WriteDATA(0x07);    //if cf from OTP   if ef from regitser 从寄存器读波形 [7:6]RES=11b:160x296；[5]REG_EN: LUT selection=1(LUT from register); [4]BWR=1 黑白运行；[3]UD=0 Gn-1->G0；[2]SHL=1 S0->Sn-1；[1]SHD_N=1 DC-DC打开；RST_N=1 no reset
    Print_I3("  \r\n");    
//    EPD_W21_Reset();        //  RESET        
//    Print_I3("---");    
//    EPD_W21_WriteCMD(0x00); // PANEL SETTING  0

...
{	
	...
	EPD_W21_WriteDATA(0x37);     // [5]REG=1(LUT from register); [4]BWR=1 黑白运行；[3]UD=0 Gn-1->G0；[2]SHL=1 S0->Sn-1；[1]SHD_N=1 DC-DC打开；RST_N=1 no reset
	EPD_W21_WriteDATA(0x01);     // 

	EPD_W21_WriteCMD(0x01);        // Power setting
	EPD_W21_WriteDATA(0x03);     // [2]=0:VDHR disable  [1]VDS_EN: Source power selection =1: Internal DC/DC function for generating VDH/VDL     [0]VDG_EN: Gate power selection =1: Internal DC/DC function for generating VGH/VGL  设置使用内部升压 
	EPD_W21_WriteDATA(0x10);     // VGH=20V, VGL= -20V
	EPD_W21_WriteDATA(0x3f);     // +15V
	EPD_W21_WriteDATA(0x3f);     // -15V
	EPD_W21_WriteDATA(0x03);     //   3V

	EPD_W21_WriteCMD(0x06);        // Booster Soft Start设定
	...
	//    EPD_W21_WriteDATA(0x09);     // border=KK D7=BDZ=1(Border output Hi-Z enabled) =0(Border output Hi-Z disabled)  D5:4=BDV[1:0]   D3=N2OCP=1(Copy NEW data to OLD data enabled)  D1:0=01(Default)  注意：这个值关系到波形表
	//    EPD_W21_WriteDATA(0x29);     // border=KW D7=BDZ=1(Border output Hi-Z enabled) =0(Border output Hi-Z disabled)  D5:4=BDV[1:0]   D3=N2OCP=1(Copy NEW data to OLD data enabled)  D1:0=01(Default)  注意：这个值关系到波形表
	//    EPD_W21_WriteDATA(0x39);     // border=LUTBD D7=BDZ=1(Border output Hi-Z enabled) =0(Border output Hi-Z disabled)  D5:4=BDV[1:0]   D3=N2OCP=1(Copy NEW data to OLD data enabled)  D1:0=01(Default)  注意：这个值关系到波形表
	...
	EPD_W21_WriteDATA(Source_Pixel_low);   // Horizontal Display Resolution=800  Low  8bit  D0~D2为0

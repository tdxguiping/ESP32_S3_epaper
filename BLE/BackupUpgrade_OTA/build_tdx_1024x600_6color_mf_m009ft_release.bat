...
:: Build all supported screen-type variants.
...
@set CUSTOMER=1
@set EPAPER_TYPE=ENABLE_INK_SCREEN_M009FT_1024X600_COLOR_6
...
@set U_FACTORY=TDX
@set U_CUSTOMER=TDX
@set U_EPAPER_TYPE_DIR=MF_M009FT_1024X600C6
::########################

:: Get current date from WMIC in YYYYMMDD format.
...
:: Enable delayed expansion inside the loop.
setlocal enabledelayedexpansion

:: Build one output for each EPD screen type.
for %%t in (%EPD_SCREEN_TYPE%) do (
    @set "EPD_SCREEN_TYPE_TMP=%%t"
	if "!EPD_SCREEN_TYPE_TMP!"=="EPD_SCREEN_TYPE_A" (
        @set U_EPAPER_TYPE=MF_M009FT_1024X600C6_A
    ) else (
        @set U_EPAPER_TYPE=MF_M009FT_1024X600C6_T
    )    
	...
	@set USER_CFLAGS=-DVER=!VER! -D!FACTORY! -DINK_SCREEN_CUSTOMER=!CUSTOMER! -D!EPAPER_TYPE! -D!FLASH_MODE! -D!EPD_SCREEN_TYPE_TMP! -DRELEASE_NO_DEBUG_PRINT -DINK_DEVICE_CHIP=INK_CHIP_CH585
	
	@echo /************************************************************/
	@echo Build Boe 1024x600 6color Project
	@echo VER:			!VER_TMP!

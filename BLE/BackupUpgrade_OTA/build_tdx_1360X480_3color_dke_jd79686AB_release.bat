...
:: ?¨ò?Dèòa±àò?μ??ùóD EPD_SCREEN_TYPE￡¨ó?????·???￡?
...
@set CUSTOMER=1
@set EPAPER_TYPE=ENABLE_INK_SCREEN_JD79686AB_1360X480_COLOR_3
...
@set U_FACTORY=TDX
@set U_CUSTOMER=TDX
@set U_EPAPER_TYPE_DIR=DKE_JD_1360X480C3
::########################
...
:: ?-?·±éàú???? EPAPER_TYPE 2￠±àò?
for %%t in (%EPD_SCREEN_TYPE%) do (
    @set "EPD_SCREEN_TYPE_TMP=%%t"
	if "!EPD_SCREEN_TYPE_TMP!"=="EPD_SCREEN_TYPE_A" (
        @set U_EPAPER_TYPE=DKE_JD_1360X480C3_A
    ) else (
        @set U_EPAPER_TYPE=DKE_JD_1360X480C3_T
    )    

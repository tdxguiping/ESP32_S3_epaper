...
show=true 表示把当前请求中的 bin 数据投递到 EPD 显示任务，并等待本次 EPD 显示任务完成；若本次 show=true 整体成功，cast/cast2pic/upload 共用处理会把 `show_control.txt` 写为 `sw=0`、停止轮播 task，并读回确认 `sw=0`。
...
- ServerNetworkStaSlideshow_StartSavedDelayed("/data") 只用于开机自动恢复轮播：先等待 `TDX_SLIDESHOW_STARTUP_DELAY_MS=10000` 毫秒，延迟结束后重新读取已保存轮播配置和控制状态。
```
...
└─ receive_data_redirect_handler()
   └─ ServerNetworkStaCast_Process()
      ...
      ├─ show=true 且整体成功时 stop_slideshow_for_cast() 并确认 sw=0
...
说明：network cast2pic 与 USB cast2pic 共享 `TdxImageTransfer_ProcessItems()` 和 `CastSaveTask`。`show` 和 `save` 是独立动作：`show=true` 时先等待 EPD 显示任务完成；`save=true` 时再提交保存任务；`show=false` 不显示，`save=false` 不保存。`show=true` 且整体成功后会停止轮播、写 `show_control.txt sw=0`，并读回确认。`cast2pic_result=0` 表示需要显示的 EPD 任务已完成、需要保存的文件已保存完成。
...
取：
- ServerNetworkStaSlideshow_StartSavedDelayed() 只用于开机自动恢复轮播：启动位置仍保持在网络初始化之后，但先等待 `TDX_SLIDESHOW_STARTUP_DELAY_MS=10000` 毫秒；等待期间手机 APP 或 USB Serial 的 cast/cast2pic/upload `show=true` 请求优先进入 EPD 显示。
- 延迟结束后重新读取 slideshow_config、control 和 `slide_progress`；如果 10 秒内 show=true 已把 control 写成 `sw=0`，则跳过自动恢复轮播。
...
说明：network upload 与 USB upload 共享 `TdxImageTransfer_ParseSingle()`、`TdxImageTransfer_ProcessItems()` 和 `CastSaveTask`。`show=true && save=true` 时先等待 EPD 显示任务完成，`save=true` 时再提交保存任务；`show=true` 且整体成功后会停止轮播、写 `show_control.txt sw=0`，并读回确认；`upload_result=0` 表示需要保存的文件已保存完成、需要显示的 EPD 任务已完成。
...
2. slideshow 开启时读取 runtime timing，扣掉当前 interval 已经走过的秒数，并额外扣掉 `TDX_SLIDESHOW_STARTUP_DELAY_MS` 换算后的秒数，向上取整，再发送 CH583 WAKE_TIMER；最小剩余时间为 1 秒。
...
说明：USB cast 与 network cast 共享 `cast_core` 和 `CastSaveTask`。USB 独有部分只保留 HTTP-like 接收、异步 worker 复制 body、以及 USB response 发送；multipart 校验、EPD 显示等待、保存 bin/jpg、记录 last_cast 与 7.1 network cast 共用同一套函数。`show=true` 且整体成功后会停止轮播、写 `show_control.txt sw=0`，并读回确认。`cast_result result=0` 表示 EPD 显示任务、保存和 last_cast 已成功。
...
说明：USB cast2pic 与 network cast2pic 共享 `TdxImageTransfer_ProcessItems()` 和 `CastSaveTask`。`screen=ab` 会先解析成 A/B 两个 image transfer item，然后逐个等待需要显示的 EPD 任务完成，再保存所有需要保存的文件；`show=true` 且整体成功后会停止轮播、写 `show_control.txt sw=0`，并读回确认；`cast2pic_result=0` 表示 EPD 显示任务和保存任务已完成。
...
```text
USB upload 与 cast 的文件字段基本一致，用于保存图片文件；如果 show=true，也会执行 EPD 显示；show=true && save=true 时先等待 EPD 显示任务完成，再保存文件；show=true 且整体成功后会停止轮播、写 `show_control.txt sw=0`，并读回确认。
```

...
network cast 的执行顺序仍然是 show=true 时先投递 EPD 队列，然后保存 bin/jpg，最后记录 last_cast。
```
...
| `ping_result` | `1405` | `Ble_MAC` 尚未从 CH583 获取；当前源码仍可返回成功和空 MAC，后续代码可按需要细分 |
...
show/save 顺序：
1. show=true 时，先把当前请求中的 bin 投递到 EPD 显示队列。
...
预期：设备返回 `cast_result`，`result=0` 表示成功；如果 `show=true`，后续会进入 EPD 显示队列。`save=false` 会返回失败，不作为“只显示不保存”的 cast 用法。
...
预期：返回 `ping_result`，`result=0`；如果返回 `Ble_MAC`，App 可用它校验当前 IP 是否是目标设备。
...
runtime 启动失败应明确反映到 result/message，代码修改时按本节 result 定义返回具体错误码。
```
...
代码修改时应把“控制文件写入成功”和“轮播 runtime 启动成功”分开判断；runtime 启动失败时，result/message 应明确提示失败原因。
```
...
预期：返回 `cast_result`，`result=0` 表示成功；`save=true` 会写 SD，`show=true` 会进入 EPD 显示队列。
...
- UsbConsoleCommon_SavePartFile() 使用 <fileName>.<ext>.tmp 临时文件，写完后 rename 成正式文件。
- UsbConsoleCommon_RecordLastCast() 写入 last cast 记录文件，路径在 /data/bin_img/ 下。

取：
- USB cast 当前不单独调用 check_cast_save_space()；保存时由 UsbConsoleCommon_SavePartFile() 直接写入文件。
- 显示时下发已收到的 bin 数据到 EPD 队列；重启恢复时可读取 last cast 记录。
```
...
建议通过 /cast、/cast2pic、/upload 覆盖 multipart 解析和文件保存，通过 /delete、/snapshot、/wifi 覆盖 JSON 解析和响应拼装。
```
...
| `test_epd_display_result` | `0` | 测试显示入队成功 |
...
预期：未知路径返回 `usb_unknown_result`；`/ping?from=router_test` 仍返回 `ping_result`。
...
| 返回 | result | 说明 |
|---|---|---|
| `wifi_result` | `0` | BLE / CH583 WiFi 配网信息已解析并保存 |
| `wifi_result` | `1301` | `ssid` 缺失 |
| `wifi_result` | `1302` | `key` 缺失 |
...
| 返回 | result | 说明 |
|---|---|---|
| `wifi_wakeup_result` | `0` | 已找到保存的 WiFi，可继续唤醒 / 连接流程 |
...
| 返回 | result | 说明 |
|---|---|---|
| `test_epd_display_result` | `0` | 测试图投递显示队列成功 |
| `test_epd_display_result` | `1801` | 当前 EPD 类型非法 |
| `test_epd_display_result` | `1803` | 测试显示失败 |
...
```text
存：
- UsbConsoleCommon_HandleImageTransfer() 在 save=true 时保存图片。
- UsbConsoleCommon_SavePartFile() 写入：/data/bin_img/<fileName>.bin。
- UsbConsoleCommon_SavePartFile() 写入：/data/jpg_img/<fileName>.jpg。
- UsbConsoleCommon_SavePartFile() 走 USB 公共保存路径，使用临时文件写入并 rename 成正式文件。
- func=cast 且 save=true 时，UsbConsoleCommon_RecordLastCast() 写入 last cast 记录文件。

取：
- UsbConsoleCommon_ExtractBoundary() 读取 multipart boundary。
- UsbConsoleCommon_MultipartParts() 读取 fileName/bin_size/image_size/bin/image。
- UsbConsoleCommon_HandleImageTransfer() 校验 bin/image 实际长度是否等于声明大小。
- show=true 时下发已收到的 bin 数据到 EPD 队列。
```

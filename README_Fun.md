...
- PC page treats either `USB console HTTP text entry ready` or `USB RX idle: waiting for PC HTTP-like request` as USB HTTP entry ready.
- PC page prints `[USB debug] auto query EPD type after serial open (...)` before the automatic `/epd_type_list` and `/epd_type` probes; if ready/idle is not seen yet, the automatic probes stay pending instead of being permanently skipped.
- Firmware prints `USB RX flush discarded ... bytes` before the USB HTTP entry becomes ready.
- Firmware prints `USB RX idle: waiting for PC HTTP-like request` every 10 seconds while the USB HTTP entry is ready but no PC request bytes have reached the ESP32-C5.
...
- `USB RX idle: waiting for PC HTTP-like request`: firmware is ready and polling USB RX, but no PC request bytes have arrived.
...
- DKE 帧数据按 30000 bytes 分包调用 spiTransmitData()，避免超过 ESP-IDF SPI 单次传输上限；任一半帧写入失败时不继续 update。

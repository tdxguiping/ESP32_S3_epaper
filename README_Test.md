...
6. 在旧WiFi recovery 30秒observer仍运行且连接处于 `RETRY_WAIT` 时执行Factory Reset。确认出现一次 `WiFi recovery cleared for Factory Reset`，白屏完成后ESP32继续运行并保持白屏；不得出现 `WiFi recovery CH583 power cycle requested`、`WiFi recovery deadline reached`、Factory Reset专用 `WAKE_TIMER`、`POWER_OFF` 或新的 `app start`。
7. 由客人正常关机后再次开机。启动时只能出现GPIO4置为HIGH并稳定100ms的日志，不得出现IO隔离、GPIO4 LOW或启动电源复位；SD挂载尝试结束后、网络图片业务启动前读取标志。实际SD卡的 `/data/welcome` 中放入唯一的 `x_welcome_0001.bin`，确认读取、解压并同步显示该welcome，成功后删除 `fr_welcome`，随后继续默认显示模式和未配网启动。EPD type、WiFi工作时间、BLE MAC和OTA状态保持不变。
...
恢复出厂无论成功或失败都不得提交Factory Reset专用关机请求。模拟文件删除失败时，应继续尝试剩余文件，最终 `file_delete_failed>0`、总 `ret!=ESP_OK`，且不设置新标志、不显示白屏。再分别模拟标志读回不一致、PSRAM不足和白屏显示失败，确认 `welcome_pending_ret`或`white_display_ret`失败且guard仍被清除；标志已保存后发生的失败必须保留标志。文件原本不存在不算失败。普通one-shot、wifi_work_time、OTA、daily和其他既有关机规则不变。

首次开机welcome的SD和彩条降级测试：

1. 放入 `x_welcome_0001.bin`和 `x_welcome_0002.bin`，确认选择编号数值更大的 `x_welcome_0002.bin`；再用编号`9`和`10`确认选择`10`，不是按字符串或目录遍历顺序选择。
2. 放入相同编号但不同前缀的两个合法文件，确认每次都按完整文件名稳定选择同一个文件，不受FAT目录顺序影响。
3. 分别测试SD卡不存在、SD挂载失败、降级挂载SPIFFS、目录不存在、没有合法文件、零字节文件、超过2MB文件、短读、损坏压缩数据和EPD显示失败；每种情况都必须释放SPI锁和welcome内存，再同步显示彩条。
4. welcome显示成功或welcome失败但彩条成功时，确认删除 `fr_welcome`，下次开机不重复显示；welcome和彩条都失败时确认保留标志，下次开机重试。
5. 注入 `fr_welcome`删除失败，确认标志保留且所有guard、LED状态和EPD资源仍被清理。
6. welcome处理期间不得启动cast、cast2pic、upload、USB图片、轮播或每日一图；处理结束后这些功能和普通网络启动必须恢复，不得因welcome失败重启或停住。
7. 在welcome目录扫描期间并发发送ping、上传和 `FACTORY_DATA`，确认ping返回EPD=BUSY，上传和厂测被拒绝；welcome结束后再次发送应恢复正常。确认日志顺序为startup welcome开始、EPD预约、SD读取、EPD显示，不能出现释放SD锁后被其他图片任务抢占的窗口。
...
17. 删除`/data/welcome/welcome_Json.txt`后测试，确认创建目录、下载当前屏型resolution的全部`images.welcome[]`，并最后写入该名称的清单。破坏本地JSON后行为相同；远端version较新或相同时都重下全部，远端version较旧时不修改任何文件。
18. 本地与远端version相同时，分别准备文件缺失、长度错误和长度正确三种情况，确认三种情况都重新下载。已有同名文件只能在新文件下载、长度校验和临时文件写入全部成功后被覆盖；下载或写入失败时原文件保持不变，且本次调用测试色块而不是显示旧BIN。
...
25. 保留同version且长度正确的BIN再次执行，确认所有welcome项仍依次出现下载及`file saved`日志，随后出现`sync complete`，并从SD精确读回显示清单第一项。清单包含多个welcome项时，确认全部文件都被重新下载和替换，但资源显示始终选择当前有效清单第一项。

...
// Keep serial output rendering lightweight by batching ANSI rendering.
// 批量渲染 ANSI 串口输出，避免每个串口包都刷新页面导致卡顿。
...
function ansiBasicColor(code, background = false) {
  const fg = {
  ...
  return background ? bg[code] : fg[code];
}
...
function ansi256Color(index) {
  ...
  const offset = n - 16;
  const r = values[Math.floor(offset / 36) % 6];
  const g = values[Math.floor(offset / 6) % 6];
  const b = values[offset % 6];
  return `#${r.toString(16).padStart(2, "0")}${g.toString(16).padStart(2, "0")}${b.toString(16).padStart(2, "0")}`;
}

function renderAnsiInline(text) {
  ...
  function applySgr(paramsText) {
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      const fg = ansiBasicColor(code, false);
      const bg = ansiBasicColor(code, true);
      if (code === 0) {
        state.bold = false;
        state.fg = "";
        state.bg = "";
        ...
        const color = ansi256Color(params[i + 2]);
  ...
  for (let match = csi.exec(input); match; match = csi.exec(input)) {
    ...
    if (match[3] === "m") applySgr(match[1].replace(/\?/g, ""));
...
function renderAnsiText(text) {
  ...
  return renderAnsiInline(plainText);
}
function flushRxOutput() {
  if (!rxPendingText) return;
  const nextText = `${rxDisplayText}${rxPendingText}`;
  rxDisplayText = nextText.length > RX_DISPLAY_MAX_CHARS ? nextText.slice(-RX_DISPLAY_MAX_CHARS) : nextText;
  rxOutput.innerHTML = renderAnsiText(rxDisplayText);
  rxPendingText = "";
